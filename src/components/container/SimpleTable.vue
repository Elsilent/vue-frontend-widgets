...
const props = withDefaults(defineProps<{
    ...
    columns: Record<string, {
        label: string;
        type: 'string'|'int'|'float'|'date'|'money'|'money_capped'|'percent'|'time';
        visible: boolean;
        colspan?: number;
    }>,
    /**
...
const columnSizingInfo = ref<Record<string, {
    height: number;
    left: number;
    width: number;
}>>({});

const visibleColumnKeys = computed(() => Object.entries(columns.value)
    .filter(([_, { visible }]) => visible)
    .map(([key, _]) => key));

const columnCountDisplayed = computed(() => visibleColumnKeys.value.length + (showRowNumber.value ? 1 : 0));
 ...
 */
const minValues = computed(() => Object.values(rows.value)
    .reduce((minValues, row) => {
        for (const [columnKey, column] of Object.entries(columns.value)) {
            ...
            if (comparisonColumnKeys.value && column.colspan > 1) {
 ...
 */
const maxValues = computed(() => Object.values(rows.value)
...
const orderedRowValues = computed(() => Object.values(rows.value)
    .map(row => ({
        ...row,
        _row_key: 0,
    }))
    ...
    if (detailsRows.value[row[primaryColumn.value]]) {
        rows.push(...detailsRows.value[row[primaryColumn.value]].map((detailRow, subindex) => ({
            ...detailRow,
            rowInfo: {
                index,
                subindex,
 ...
 */
const rowValues = computed(() => Object.values(rows.value));
 ...
 */
const getColorizedButtonTopOffset = (columnKey) => {
 ...
 */
const getCellClasses = (row, rowIndex, columnKey, subcolumnIndex = undefined) => {
    const classes = {
        ...cellClasses.value,
        ...{ even: rowIndex % 2 === 0 },
    };
    ...
    if (colorMetrics.value) {
        const colorMood = inversedKpis.value.includes(columnKey)
            ? 'negative'
            : 'positive';

        classes[`color-intensity-${getColorIntensity(row[columnKey], columnKey, subcolumnIndex)}`] = true;
        classes[`color-${colorMood}`] = true;

        if (subcolumnIndex !== undefined && comparisonColumnKeys.value[subcolumnIndex] === 'difference') {
 ...
 */
const getColorIntensity = (value, columnKey, subcolumnIndex = undefined) => {
 ...
 */
const getColumnClassList = (columnKey, subcolumnIndex = undefined) => {
    const column = columns.value[columnKey];
    ...
    const ordered = orderBy.value[0]
        ? orderBy.value[0].every((orderBySegment, segmentIndex) => orderBySegment === orderByKey[segmentIndex])
        : false;

    return {
        'column-main-extended': subcolumnIndex === undefined && column.rowspan > 1,
        dragged: visibleColumnKeys.value[dragColumnFromIndex.value] === columnKey,
        'drag-mode': dragModeEnabled.value,
        orderable: !comparisonColumnKeys.value || (column.colspan ?? 1) === 1 || subcolumnIndex !== undefined,
        'ordered-by': ordered,
 ...
 */
const getColumnGhostStyle = (columnKey) => {
    const style = {};

    if (columnSizingInfo.value[columnKey]) {
        if (dragColumnFromIndex.value !== undefined && visibleColumnKeys.value[dragColumnFromIndex.value] === columnKey) {
 ...
 */
const getOrderByKey = (columnKey, subcolumnIndex = undefined) => {
    if (subcolumnIndex === undefined) {
        return columnKey;
    } else {
        return comparisonColumnKeys.value
            ? `${columnKey}-${comparisonColumnKeys.value[subcolumnIndex]}`
 ...
 */
const getRowNumberValue = (rowInfo) => rowInfo.subindex === undefined
    ? rowInfo.index + 1
 ...
 */
const indexOfColored = (columnKey) => coloredMetrics.value.findIndex((kpi) => kpi === columnKey);

/**
 * Checks if a column can be colored
 */
const isColorable = (columnKey) => colorMetrics.value
    && !['date', 'string'].includes(columns.value[columnKey].type);

const emit = defineEmits<{
    (e: 'move:column', from: number, to: number): void;
 ...
 */
const onColumnClick = (columnKey, subcolumnIndex = undefined) => {
    if (comparisonColumnKeys.value && columns.value[columnKey].colspan && subcolumnIndex === undefined) {
        return;
    }
    ...
    const alreadyOrderedBy = newOrderBy
        .every((orderBySegment, segmentIndex) => orderBySegment === orderBy.value[0][segmentIndex]);
 ...
 */
const onColumnDragStart = (columnKey, event) => {
    ...
    window.addEventListener('mousemove', onColumnDragMove.value);
    window.addEventListener('mouseup', onColumnDragStop.value);
};
 ...
 */
const onColumnDragStop = (event) => {
    if (dragColumnFromIndex.value === undefined) {
        return;
    }

    const shouldUpdate = dragModeEnabled.value && dragColumnToIndex.value !== draggedColumn.value;
    ...
    dragElementTarget.value = undefined;
    dragColumnToIndex.value = undefined;
    dragMoves.value = 0;
    dragColumnFromIndex.value = undefined;

    window.removeEventListener('mousemove', onColumnDragMove.value);
    window.removeEventListener('mouseup', onColumnDragStop.value);
};

/**
 * Toggles column colored state
 */
const toggleColored = (columnKey) => {
...
.table(
  ...
  template(v-for="columnKey in visibleColumnKeys")
    .cell.column.column-main(
      :ref="(el) => columnRefs[columnKey] = el",
      ...
      .toggle-colored.no-spacing(
        ...
        slot(name="colorizeLabel", enabled="indexOfColored(columnKey) >= 0")
  //- Renders "ghost" cells
